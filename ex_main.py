...
while True:
    ...
    # 发送get请求
    r = requests.get("https://www.imdb.com/title/tt0111161/reviews/_ajax?ref_=undefined&paginationKey=" + paginationKey,
    ...
    for i in range(len(ct)):
        ...
        # 将获取的评论添加到reviews_all列表中
        reviews_all.append({"title": title, "review": review, "rate": rate})
    if len(reviews_all) > 100:
        break
    print("共获取了" + str(len(reviews_all)) + "条评论")
    # 将结果写入reviews.txt
    file = open("reviews" + str(num) + ".txt", "a", encoding="utf8")
